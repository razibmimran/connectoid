...
public class Orders {
		...
		while (processed < n_orders) {
			...
			if ((order_flags & RDP_ORDER_SECONDARY) != 0) {
				...
				case RDP_ORDER_DESKSAVE:
					...
					this.processDeskSave(data, os.getDeskSave(), present,
									delta);
					break;
				...
				case RDP_ORDER_POLYLINE:
					...
					this.processPolyLine(data, os.getPolyLine(), present,
									delta);
					break;
		...
		if (compressed) {
			...
			for (y = 0; y < height; y++)
				data.copyToByteArray(bmpdata, y * (width * Bpp),
						(height - y - 1) * (width * Bpp), width * Bpp); //memcpy(
			// &
			// bmpdata
			// [(
			// height
	...
	private void processRectangle(RdpPacket_Localised data,
		...
		if ((present & 0x10) != 0)
			this.rect_colour = (this.rect_colour & 0xffffff00) | data.get8(); // rect
		// .
		// setColor
		// (
		// setColor
