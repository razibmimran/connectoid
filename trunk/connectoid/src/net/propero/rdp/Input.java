...
import org.apache.log4j.Level; // TODO(pv):
...
public abstract class Input {
	...
	private boolean isMenuPressed(KeyEvent e) {
		return (e.getKeyCode() == KeyEvent.KEYCODE_MENU);
	}
	...
	public void clearKeys() {
		...
		if (lastKeyEvent.isAltPressed() || serverAltDown) {
			...
			sendScancode(getTime(), RDP_KEYRELEASE | KBD_FLAG_QUIET, 0x38); //l.
																			// alt
		}
		if (isMenuPressed(lastKeyEvent)) {
	...
	class RdesktopKeyAdapter extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			pressedKeys.addElement(new Integer(keyCode));

			logger.debug("PRESSED keycode=0x" + Integer.toHexString(keyCode)
					+ " char='" + ((char) keyCode) + "'");
		...
		public void keyTyped(KeyEvent e) {
			...
			logger.debug("TYPED keycode=0x" + Integer.toHexString(keyCode)
					+ " char='"	+ ((char)keyCode) + "'");
		...
		public void keyReleased(KeyEvent e) {
			...
			// it
			int keyCode = new Integer(e.getKeyCode());
			...
			logger.debug("RELEASED keycode=0x" + Integer.toHexString(keyCode)
					+ " char='" + ((char)keyCode) + "'");
			if (rdp != null) {
	...
	public boolean handleShortcutKeys(long time, KeyEvent e, boolean pressed) {
		...
		case KeyEvent.VK_INSERT: // Alt + Insert = Alt + Esc
			...
			break;
		*/
		case KeyEvent.KEYCODE_DEL: // Alt + Delete = Menu
			if (pressed) {
				sendScancode(time, RDP_KEYRELEASE, 0x38); // ALT
	...
	public boolean handleSpecialKeys(long time, KeyEvent e, boolean pressed) {
		...
		case KeyEvent.KEYCODE_ALT_RIGHT:
			...
			return false;
		/*
		// TODO(pv): Need a popup for these special keys
		// There is no "Caps Lock" key, its a mode enabled if Shift is pressed twice 
	...
	public boolean onTrackballEvent(MotionEvent e) {
		// TODO(pv): Need menu option for Right (2) or Middle (3) click
	 ...
	 *            was pressed
	 */
	protected void middleButtonPressed(MotionEvent e) {
		/*
		...
		rdp.sendInput(time, RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON3
				| MOUSE_FLAG_DOWN, (int)e.getX(), (int)e.getY());
	}
	 ...
	 *            was released
	 */
	protected void middleButtonReleased(MotionEvent e) {
		/* if (!Options.paste_hack || !ctrlDown) */
		rdp.sendInput(time, RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON3, (int)e.getX(), (int)e.getY());
	}
	...
	class RdesktopMouseMotionAdapter extends MouseMotionAdapter {
		...
		public void mouseMoved(MotionEvent e) {
			...
			if (rdp != null) {
				rdp.sendInput(time, RDP_INPUT_MOUSE, MOUSE_FLAG_MOVE, (int)e.getX(), (int)e.getY());
			}
		}
