...
public class Rdp {
	...
	private void sendLogonInfo(int flags, String domain, String username,
		...
		if (!Options.use_rdp5 || 1 == Options.server_rdp_version) {
			...
			// + 2);
			data.outUnicodeString("C:\\WINNT\\System32\\mstscax.dll", len_dll); // rdp_out_unistr
																				// (
																				// s
																				// ,
			...
			data.incrementPosition(62 - 2 * "GTB, normaltid".length()); // out_uint8s
																		// (s,
			// 62 -
			// 2 *
			// strlen("GTB,
			...
			data.incrementPosition(62 - 2 * "GTB, sommartid".length()); // out_uint8s
																		// (s,
			// 62 -
			// 2 *
			// strlen("GTB,
			...
			data.setLittleEndian32(Options.rdp5_performanceflags); // out_uint32_le
																	// (s,
			// 0x0f);
			data.setLittleEndian32(0); // out_uint32(s, 0);
		}
	...
	private void sendConfirmActive() throws RdesktopException, IOException,
		...
		this.sendUnknownCaps(data, 0x0d, 0x58, caps_0x0d); // rdp_out_unknown_caps
															// (s,
		// 0x0d, 0x58,
		// caps_0x0d); /*
		// international? */
		this.sendUnknownCaps(data, 0x0c, 0x08, caps_0x0c); // rdp_out_unknown_caps
															// (s,
		// 0x0c, 0x08,
		// caps_0x0c);
		this.sendUnknownCaps(data, 0x0e, 0x08, caps_0x0e); // rdp_out_unknown_caps
															// (s,
		// 0x0e, 0x08,
		// caps_0x0e);
		this.sendUnknownCaps(data, 0x10, 0x34, caps_0x10); // rdp_out_unknown_caps
															// (s,
		// 0x10, 0x34,
		// caps_0x10); /*
		// glyph cache? */
