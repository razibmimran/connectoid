...
public class Secure {
	...
	public RdpPacket_Localised sendMcsData() {
		...
		if (Options.use_rdp5) {
			...
			buffer.setLittleEndian32(Options.console_session ? 0xb : 0xd); // out_uint32_le
																			// (
																			// s
																			// ,
																			// g_console_session
		...
		if (Options.use_rdp5 && (channels.num_channels() > 0)) {
			...
			buffer.setLittleEndian16(SEC_TAG_CLI_CHANNELS); // out_uint16_le(s,
															...
															// );
			buffer.setLittleEndian16(channels.num_channels() * 12 + 8); // out_uint16_le
																		// (s,
																		// g_num_channels
																		// * 12
																		// + 8);
			...
			for (int i = 0; i < channels.num_channels(); i++) {
				...
				buffer.out_uint8p(channels.channel(i).name(), 8); //out_uint8a(s
																	...
																	// [i].name,
																	// 8);
				buffer.setBigEndian32(channels.channel(i).flags()); // out_uint32_be
																	// (s,
																	// g_channels
																	// [
																	// i].flags)
	...
	private void processSrvInfo(RdpPacket_Localised mcs_data) {
		Options.server_rdp_version = mcs_data.getLittleEndian16(); // in_uint16_le
																	// (s,
